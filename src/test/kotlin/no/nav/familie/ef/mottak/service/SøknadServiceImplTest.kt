...
import no.nav.familie.ef.mottak.no.nav.familie.ef.mottak.service.Testdata.skjemaForArbeidssøker
import no.nav.familie.ef.mottak.no.nav.familie.ef.mottak.service.Testdata.søknad
...
internal class SøknadServiceImplTest : IntegrasjonSpringRunnerTest() {
    ...
    internal fun `lagre skjema for søknad`() {
        val kvittering = søknadService.motta(SøknadMedVedlegg(søknad, emptyList()))
        val søknad = søknadService.get(kvittering.id)
        assertThat(søknad).isNotNull
    }
}
